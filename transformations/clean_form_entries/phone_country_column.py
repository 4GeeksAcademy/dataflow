...
def run(df):
    ...
    print('Shape before new phone_country column ', df.shape)

    df['phone'] = df['phone'].apply(lambda x: f'{x:.0f}')
    df['phone'] = df['phone'].fillna(0)

    #create a list to get all countries obtained from phone number
    ...
    #Assign that list to a new column named phone_country

    df['phone_country'] = correct_country

    #Verify new column has been created
                  
    print('Shape after new phone_country column ', df.shape)
